"""v3 baseline(peak8) vs v3+adaptive20 비교.
...
NEW = "outputs/eval/analyze_qwen_fp_removal_61_v3_adaptive20.json"  # v3 adaptive20
OUT_CSV = "outputs/eval/compare_v3_adaptive20_changes.csv"
...
def main():
    ...
    print("=" * 64)
    print("[성능] v3 baseline(peak8) vs v3+adaptive20 (61, GT-centric, IoU>=0.10)")
    print("| Setting          | TP | FP | FN | Prec | Rec | F1 |")
    print("|------------------|----|----|----|------|-----|-----|")
    for name, d in [("v3 peak8 (base)", bd), ("v3 adaptive20", nd)]:
    ...
    # adaptive 샘플링 모드 사용 통계
    used = {}
    for r in nmap.values():
        used[r.get("sampling_used")] = used.get(r.get("sampling_used"), 0) + 1
    print(f"  sampling_used: {used}  (uniform=긴 이벤트, peak=짧은 이벤트)")
